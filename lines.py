...
def lines(args, dictionary, letters):
    """The function that runs the spell checker and outputs the line with the correct words"""
    words_read = 0
    unknown = []
    incorrect = []
    # Reads the input
    ...
    for i in range(len(line_list)):
        ...
        for word in word_list:
            ...
            # Checks to see if the lowercase word is in the dictionary, if it is adds it to the correction line
            if not sp.inDict(new_word, dictionary):
                ...
                # Adds the fixed or unknown word back to the line
                line = line + fix_word + " "
            else:
                line = line + word + " "
        print(line)
